...
public class Regras {
    ...
    public List<Jogada> jogadasPossiveis(Peça peça){
        ...
        Posição posiçãoPeça = tabuleiro.getPosição(peça);
        ...
        Posição pos1 = new Posição(posiçãoPeça.getI() + varJogador, posiçãoPeça.getJ() - 1);
        Posição pos2 = new Posição(posiçãoPeça.getI() + varJogador, posiçãoPeça.getJ() + 1);

        //Posições que podem conter inimigos e que estão no sentido contrário ao "normal" da peça.
        Posição pos3 = new Posição(posiçãoPeça.getI() - varJogador, posiçãoPeça.getJ() - 1);
        Posição pos4 = new Posição(posiçãoPeça.getI() - varJogador, posiçãoPeça.getJ() + 1);
        
        //Verifica se as posições são válidas.
        if (jogadaValida(pos1, peça)) {
            posicoes.add(pos1);
            //Candidata a ser uma jogada "normal". Pode não ser.
            jogadas.add(new Jogada(null, peça, posiçãoPeça, pos1));
        }

        if (jogadaValida(pos2, peça)) {
            posicoes.add(pos2);
            //Candidata a ser uma jogada "normal". Pode não ser.
            jogadas.add(new Jogada(null, peça, posiçãoPeça, pos2));
        }
        ...
        if (jogadaValida(pos3, peça)) {
            posicoes.add(pos3);
            if(peça.isDama())
                jogadas.add(new Jogada(null, peça, posiçãoPeça, pos3));
        }

        if (jogadaValida(pos4, peça)) {
            posicoes.add(pos4);
            if(peça.isDama())
                jogadas.add(new Jogada(null, peça, posiçãoPeça, pos4));
        }
        ...
        if(peça.isDama()){
            for (int cont = 0; cont < 4; cont++) {
                switch(cont){
                    case 0:
                        if(pos1.getI()>=0 && pos1.getJ()>=0){
                            Posição origem = new Posição(pos1.getI() , pos1.getJ());
                            for (int k = 1; k < Tabuleiro.DIMEN; k++) {
                                Posição atual = new Posição(origem.getI()+varJogador , origem.getJ()-1);
                                origem = atual;
                                if (jogadaValida(atual, peça)) {
                                    posicoes.add(atual);
                                    //Candidata a ser uma jogada "normal". Pode não ser.
                                    jogadas.add(new Jogada(null, peça, posiçãoPeça, atual));
                                }
                            }
                        }
                        break;
                    case 1:
                        if(pos2.getI()>=0 && pos2.getJ()>=0){
                            Posição origem = new Posição(pos2.getI() , pos2.getJ());
                            for (int k = 1; k < Tabuleiro.DIMEN; k++) {
                                Posição atual = new Posição(origem.getI()+varJogador , origem.getJ()+1);
                                origem = atual;
                                if (jogadaValida(atual, peça)) {
                        ...
                        break;
                    case 2:
                        if(pos3.getI()>=0 && pos3.getJ()>=0){
                            Posição origem = new Posição(pos3.getI() , pos3.getJ());
                            for (int k = 1; k < Tabuleiro.DIMEN; k++) {
                                Posição atual = new Posição(origem.getI()-varJogador , origem.getJ()-1);
                                origem = atual;
                                if (jogadaValida(atual, peça)) {
                        ...
                        break;
                    case 3:
                        if(pos4.getI()>=0 && pos4.getJ()>=0){
                            Posição origem = new Posição(pos4.getI() , pos4.getJ());
                            for (int k = 1; k < Tabuleiro.DIMEN; k++) {
                                Posição atual = new Posição(origem.getI()-varJogador , origem.getJ()+1);
                                origem = atual;
                                if (jogadaValida(atual, peça)) {
    ...
    public Posição podeComer(Peça peca1, Peça peca2){
        ...
        if(peca1.getTime() == JOGADOR_UM){
            ...
            //Verifico se existe alguma peça nessa posição.
